...
namespace Talent.Services.Profile.Domain.Services
{
    public class ProfileService : IProfileService
    {
        ...
        public async Task<TalentProfileViewModel> GetTalentProfile(string Id)
        {
            ...
            // Create ViewModel and set fields to their ViewModel equivalent values.
            var talentProfile = await ViewModelFromUser(user);
        ...
        public async Task<IEnumerable<TalentSnapshotViewModel>> GetTalentSnapshotList(string employerOrJobId, bool forJob, int position, int increment)
        {
            ...
            try
            {
                // We want a collection with just the ids of suggested talents.
                var suggestions = _employerRepository.GetQueryable()
                    .Where(x => x.Id == employerOrJobId)
                    .SelectMany(x => x.Suggestions)
                    .Skip(increment * position)
                    .Take(increment)
                    .Select(x => x.TalentId)
                    .ToArray();

                // We can't do a transformation to the TalentSnapshotViewModel right away
                // as the MongoDB driver doesn't support that kind of transformation.
                var talents = _userRepository.GetQueryable()
                    .Where(x => suggestions.Contains(x.Id))
                    .ToList();

                // We'll do the transformation now.
                var talentSnapshots = new List<TalentSnapshotViewModel>(talents.Count);
                foreach (var talent in talents)
                {
                    var snapshot = await ViewModelSnapshotFromUser(talent);
                    talentSnapshots.Add(snapshot);
                }
            ...
            {
                throw new Exception("Error communicating with the database");
            }
        }
        ...
        /// <summary>
        /// Takes a User and returns a ViewModel equivalent of it. 
        /// However it doesn't map some member fields as they require their own ViewModels.
        /// You must set Languages, Skills, Education, Certifications and Experience fields yourself.
        /// </summary>
        ...
        protected async Task<TalentProfileViewModel> ViewModelFromUser(User user)
        {
            string videoUrl = string.IsNullOrWhiteSpace(user.VideoName) 
                                ? ""
                                : await _fileService.GetFileURL(user.VideoName, FileType.UserVideo);

            string cvUrl = string.IsNullOrWhiteSpace(user.CvName)
                                ? ""
                                : await _fileService.GetFileURL(user.CvName, FileType.UserCV);
        ...
        /// <summary>
        /// Takes a User and returns a TalentSnapshotViewModel equivalent of it.
        /// You must set VideoUrl, CVUrl and Level fields yourself.
        /// </summary>
        /// <param name="user"></param>
        /// <returns></returns>
        protected async Task<TalentSnapshotViewModel> ViewModelSnapshotFromUser(User user)
        {
            string videoUrl = string.IsNullOrWhiteSpace(user.VideoName)
                                ? ""
                                : await _fileService.GetFileURL(user.VideoName, FileType.UserVideo);
            ...
            var lastEmployment = user.Experience.LastOrDefault();
            string employment = lastEmployment.End > DateTime.Now ? $"{lastEmployment.Position} at {lastEmployment.Company}" : "";

            string visa = user.VisaStatus;
            if (user.VisaExpiryDate.HasValue)
            {
                visa += $" - {user.VisaExpiryDate}";
            }
            ...
            return new TalentSnapshotViewModel
            {
                ...
                Visa = visa,
                Level = "", // What level is this meant to be?
                Skills = user.Skills.Select(x => x.Skill).ToList()
            };
        }
